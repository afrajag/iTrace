...
class SpectralCurve {
    static var WAVELENGTH_MIN: Int32 = 360
    static var WAVELENGTH_MAX: Int32 = 830

    static var CIE_xbar: [Double] = [0.000129900000, 0.000232100000,
                                      ...
                                      0.000001251141]
    static var CIE_ybar: [Double] = [0.000003917000, 0.000006965000,
                                      ...
                                      0.000000451810]
    static var CIE_zbar: [Double] = [0.000606100000, 0.001086000000,
    ...
    static var WAVELENGTH_STEP: Int32 = (WAVELENGTH_MAX - WAVELENGTH_MIN) / Int32(CIE_xbar.count - 1)
    ...
    func sample(_: Float) -> Float {
        return 0.0
    }

    // self check here
    init() {
        UI.printDetailed(.IMG, "Checking Spectral Curve coherence ...")

        if Self.WAVELENGTH_STEP * Int32(Self.CIE_xbar.count - 1) != Self.WAVELENGTH_MAX - Self.WAVELENGTH_MIN {
            fatalError("Internal error - spectrum static data is inconsistent\n  * min = \(Self.WAVELENGTH_MIN)\n  * max = \(Self.WAVELENGTH_MAX)\n  * step = \(Self.WAVELENGTH_STEP)\n  * num = \(Self.CIE_xbar.count)")
        }
    }
    ...
    func toXYZ() -> XYZColor {
        ...
        var w = Self.WAVELENGTH_MIN

        for i in 0 ..< Self.CIE_xbar.count {
            let s: Float = sample(Float(w))

            X += s * Float(Self.CIE_xbar[i])
            Y += s * Float(Self.CIE_ybar[i])
            Z += s * Float(Self.CIE_zbar[i])

            w += Self.WAVELENGTH_STEP
        }

        return XYZColor(X, Y, Z).mul(Float(Self.WAVELENGTH_STEP))
    }
}
