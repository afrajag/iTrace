...
final class AccelerationStructurePlugins: Plugins {
    var pluginClasses: [String: AccelerationStructure.Type]

    required init() {
        pluginClasses = [String: AccelerationStructure.Type]()
    }

    func createInstance(_ name: String?) -> AccelerationStructure? {
        if name == nil || name == "none" {
            UI.printError(.SYS, "'\(name ?? "no_value")' Type not present !")
    ...
    @discardableResult
    func registerPlugin(_ name: String, _ pluginClass: AccelerationStructure.Type) -> Bool {
