...
final class TestScene: SceneBuilder {
    func build() -> SceneParameter? {
        let scene = SceneParameter("TestScene")
        ...
        */
        ...
        let ibl: ImageBasedLightParameter = ImageBasedLightParameter("ibl_0")
        ibl.texture = "matprev_env.hdr"
        ibl.center = Vector3(0, -1, 0)
        ibl.up = Vector3(0, 0, 1)
        ibl.fixed = true
        ibl.samples = 200
        scene.add(ibl)
