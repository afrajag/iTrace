...
final class GlobalPhotonMap : GlobalPhotonMapInterface {
	...
	var hasRadiance: Bool = false
	var maxPower: Float = 0.0
	var maxRadius: Float = 0.0
	var _numEmit: Int32 = 0
	
    let lockQueue = DispatchQueue(label: "globalphotonmap.lock.serial.queue")
	...
	func store(_ state: ShadingState, _ dir: Vector3, _ power: Color, _ diffuse: Color) {
        let p: Photon = Photon(state.getPoint(), state.getNormal()!, dir, power, diffuse)
	...
	func initStore() {
		UI.printInfo(.LIGHT, "Balancing global photon map ...")
        ...
        UI.printInfo(.LIGHT, "Global photon map:")
		UI.printInfo(.LIGHT, "  * Photons stored:   \(storedPhotons)")
		UI.printInfo(.LIGHT, "  * Photons/estimate: \(numGather)")
		UI.printInfo(.LIGHT, "  * Estimate radius:  \(gatherRadius)")
        
        maxRadius = 1.4 * sqrt(maxPower * Float(numGather))
        ...
        UI.printInfo(.LIGHT, "  * Precompute time:  \(t.toString())")
		UI.printInfo(.LIGHT, "  * Radiance photons: \(storedPhotons)")
		UI.printInfo(.LIGHT, "  * Search radius:    \(gatherRadius)")
	}

	func precomputeRadiance() {
		if storedPhotons == 0 {
			return
		}
		...
		let quadStoredPhotons: Int32 = halfStoredPhotons / 2
		let p: Point3 = Point3()
		let n: Vector3 = Vector3()
		let ppos: Point3 = Point3()
		let pdir: Vector3 = Vector3()
		let pvec: Vector3 = Vector3()
		let irr: Color = Color()
		let pow: Color = Color()
		let maxDist2: Float = gatherRadius * gatherRadius
		
        let np: NearestPhotons = NearestPhotons(p, numGather, maxDist2)
        
        var temp: [Photon?]? = [Photon?](repeating: nil, count: Int(quadStoredPhotons) + 1)
		
        UI.taskStart("Precomputing radiance", 1, quadStoredPhotons)
		
        for i in 1 ... quadStoredPhotons {
			UI.taskUpdate(i)
            
            let curr: Photon = photons![Int(i)]!
			
            p.set(curr.x, curr.y, curr.z)
			
            n.set(Vector3.decode(curr.normal))
            ...
            for j in 1 ... np.found {
                let phot: Photon = np.index[Int(j)]!
				
                pdir.set(Vector3.decode(phot.dir))
				
                let cos: Float = -Vector3.dot(pdir, n)
				
                if cos > 0.01 {
					ppos.set(phot.x, phot.y, phot.z)
					
                    pvec.set(Point3.sub(ppos, p))
					
                    let pcos: Float = Vector3.dot(pvec, n)
            ...
            //  compute radiance
            irr.mul(Color(curr.data)).mul(1.0 / Float.pi)
	...
	func getRadiance(_ p: Point3, _ n: Vector3) -> Color {
		...
		var cameFrom: Int32
		var dist2: Float
		var maxDist2: Float = gatherRadius * gatherRadius
        var nearest: Photon? = nil
		var curr: Photon
		let photN: Vector3 = Vector3()
        ...
        while true {
			while i < halfStoredPhotons {
                let dist1d: Float = photons![Int(i)]!.getDist1(px, py, pz)
            ...
            if dist2 < maxDist2 {
                photN.set(Vector3.decode(curr.normal))
				
                let currentDotN: Float = Vector3.dot(photN, n)
			...
			repeat {
				...
				if i == 0 {
					return (nearest == nil ? Color.BLACK : Color().setRGBE(nearest!.data))
				}
            } while (dist1d2[Int(level)] >= maxDist2) || (cameFrom != chosen[Int(level)])
